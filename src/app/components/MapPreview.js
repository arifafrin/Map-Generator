...
export default memo(function MapPreview({ 
  ...
  const defs = (
    <defs>
      ...
      {/* Removed the clipPath dependency entirely for maskless native shape rendering */}
  ...
  const renderPath = (path, globalIdx) => {
    ...
    let displayStroke = isHovered ? '#ffffff' : styleConfig.stroke;
    // Hide the base outline stroke completely for Radial Halftone for a clean shape pattern
    if (styleConfig.isRadialDotted) {
        displayStroke = 'transparent';
    }
    ...
    if (styleConfig.isRadialDotted && hBounds) {
       ...
       // Parse discrete vertices from path.d for rigorous Point-in-Polygon intersection testing
       const matches = path.d.match(/[ML]\s*(-?[\d.]+)[,\s]+(-?[\d.]+)/g);
       const vertices = [];
       if (matches) {
           for (let i = 0; i < matches.length; i++) {
               const coords = matches[i].match(/-?[\d.]+/g);
               if (coords && coords.length >= 2) {
                   vertices.push({x: parseFloat(coords[0]), y: parseFloat(coords[1])});
               }
           }
       }
       ...
       for (let x = b.minX; x <= b.maxX; x += spacing) {
         for (let y = b.minY; y <= b.maxY; y += spacing) {
            ...
            // Ray-Casting Point-in-Polygon Check to guarantee perfectly whole shapes without clipping masks
            let inside = false;
            if (vertices.length > 2) {
                ...
                inside = true; // Safe fallback strictly for purely rounded shapes lacking sufficient M/L anchors
            }
            if (!inside) continue;
            ...
            if (r > 0.4) {
               if (halftoneShape === 'heart') {
                   const scale = r / 10;
                   // Render scalable Heart 'love shape' using a mathematically precise zero-centered Bezier construct
                   dots.push(
                      <path 
                         key={`rdot-${x}-${y}`} 
                         d="M0 10.35 l-1.45-1.32 C-6.6 4.36 -10 1.28 -10 -2.5 C-10 -5.58 -7.58 -8 -4.5 -8 c1.74 0 3.41 0.81 4.5 2.09 C1.09 -7.19 2.76 -8 4.5 -8 C7.58 -8 10 -5.58 10 -2.5 c0 3.78 -3.4 6.86 -8.55 11.54 L0 10.35 Z"
                         transform={`translate(${x}, ${y}) scale(${scale.toFixed(4)})`}
                         fill={path.fillColor} 
                      />
                   );
               } else if (halftoneShape === 'square') {
                   dots.push(<rect key={`rdot-${x}-${y}`} x={x - r} y={y - r} width={r * 2} height={r * 2} fill={path.fillColor} />);
               } else if (halftoneShape === 'diamond') {
                   dots.push(<polygon key={`rdot-${x}-${y}`} points="0,-1 1,0 0,1 -1,0" transform={`translate(${x}, ${y}) scale(${r})`} fill={path.fillColor} />);
               } else {
                   // Default circle dot
